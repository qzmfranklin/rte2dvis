...
#
#  The ${DIR-test_mem_bw/omp_trans_mem_bw} variable will be used through the rest of this 
...
${DIR-test_mem_bw/omp_trans_mem_bw}CXXFLAGS:=${CXXFLAGS}
${DIR-test_mem_bw/omp_trans_mem_bw}INCS:=${INCS}
###############################################################################
#				STEP 3
#DIRECTORY-SPECIFIC SOURCE, OBJECT, and LIBRARY FILES
#
#  Remember to add the ${DIR-test_mem_bw/omp_trans_mem_bw}/ to whatever source files(s) you wish to 
#  add.
...
${DIR-test_mem_bw/omp_trans_mem_bw}CFILES:= \
	${DIR-test_mem_bw/omp_trans_mem_bw}/trans.c
...
#  Linux system.
${DIR-test_mem_bw/omp_trans_mem_bw}LIBFILES:=
###############################################################################
#				STEP 4
...
#  libraries are still preserved in the top level Makefile.vars for use in a 
#  Linux system.
${DIR-test_mem_bw/omp_trans_mem_bw}BINLIB:=
###############################################################################
#				STEP 5
#DIRECTORY-SPECIFIC TEST FILES
#  Speicify all the test files. All test files must be CPP files. But when
#  listing the them in ${DIR-test_mem_bw/omp_trans_mem_bw}TST, do NOT write the .cpp extension. 
#  For example, if one wishes to add test_mytest.cpp, he should write:
#  		${DIR-test_mem_bw/omp_trans_mem_bw}TST:=test_mytest
#  Then list all the build rules right afterwards, e.g.:
#		${BUILD}/test_mytest.exe: ${BUILD}/test_mytest.o \
#					  ${BUILD}/any-other-files.o
${DIR-test_mem_bw/omp_trans_mem_bw}TST:=test_trans\
	time_trans
${BUILD}/test_trans.exe: ${BIN}/c_utils.dylib ${BIN}/cpp_utils.dylib \
	${BUILD}/test_trans.o ${BUILD}/trans.o 
${BUILD}/time_trans.exe: ${BIN}/c_utils.dylib ${BIN}/cpp_utils.dylib \
	${BUILD}/time_trans.o ${BUILD}/trans.o
###############################################################################
#				STEP 6
...
################## DO NOT MODIFY ################
SRCFILES:=${SRCFILES} ${${DIR-test_mem_bw/omp_trans_mem_bw}CFILES} \
	${${DIR-test_mem_bw/omp_trans_mem_bw}CPPFILES} ${${DIR-test_mem_bw/omp_trans_mem_bw}TSTCPP} \
	${${DIR-test_mem_bw/omp_trans_mem_bw}BINCPP}
DEPFILES:=${DEPFILES} ${${DIR-test_mem_bw/omp_trans_mem_bw}DEPFILES} \
	${${DIR-test_mem_bw/omp_trans_mem_bw}TSTDEP} ${${DIR-test_mem_bw/omp_trans_mem_bw}BINDEP}
################## DO NOT MODIFY ################
###############################################################################
###############################################################################
#				WANRING
...
${DIR-test_mem_bw/omp_trans_mem_bw}-all: ${${DIR-test_mem_bw/omp_trans_mem_bw}OBJFILES} ${${DIR-test_mem_bw/omp_trans_mem_bw}BINEXE}		\
	${${DIR-test_mem_bw/omp_trans_mem_bw}LIBFILES} ${${DIR-test_mem_bw/omp_trans_mem_bw}BINLIB}
...
${DIR-test_mem_bw/omp_trans_mem_bw}-list:
	...
	@$(foreach dir, 						\
		...
		,							\
		if [ ! -z "${${DIR-test_mem_bw/omp_trans_mem_bw}${dir}}" ]; then 		\
			echo "${BROWN}${dir}${NONE}\t\c";		\
			$(foreach file,${${DIR-test_mem_bw/omp_trans_mem_bw}${dir}},		\
			    if [ -f ${file} ]; then echo		\
				"${BLACK}${file}${NONE}\c";		\
