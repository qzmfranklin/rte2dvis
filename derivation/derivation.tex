...
In this section, we advance to single-point testing and multi-point source.
...
\begin{equation} \label{eq:result-Bnmnpmp-quadrature}
	\begin{split}
		...
		=& 
			...
			\right]
			\frac
			{ e^{+i (m^\prime - m)\phi_{\vectorsym{r}_n
			-\vectorsym{r}_{{j_{n^\prime}}}}} }
			{\lvert\vectorsym{r}_n-\vectorsym{r}_{{j_{n^\prime}}}\rvert}
	\end{split}
\end{equation}
The $M_{n^\prime}$ is the number of sampling points for $S_{n^\prime}$.
\\

The precision for any specific $B_{(nm)(n^\prime m^\prime)}$ depends on the following three factors:
\begin{itemize}
	\item	the distance between the source and the testing triangle, and
	\item	the order of the simple harmonics involved, $\lvert m - m^\prime \rvert$, and
	\item	the quadrature rule, i.e., 
		$\left\{ (\xi_i,\zeta_i), w_i \right\} _{i=1}^{M}$,
		being employed.
\end{itemize}
