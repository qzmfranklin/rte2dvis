...
Use the quadrature rules to evaluate the $V_{sc}$ in
Eq.\eqref{eq:der-Vscnm-1pt}:
...
\begin{align}
	\begin{split}
		...
		=&
			...
			\mu_s(\vectorsym{r}_{{j_{n^\prime}}})
			\frac
			{ e^{-i m\phi_{\vectorsym{r}_{j_n}
			-\vectorsym{r}_{{j_{n^\prime}}}}} }
			{\lvert\vectorsym{r}_{j_n}-\vectorsym{r}_{{j_{n^\prime}}}\rvert}
			f(\phi_{\vectorsym{r}_{j_n}-\vectorsym{r}_{j_n^\prime}}-\phi^I)
			e^{\tau(\vectorsym{r}_{j_n^\prime},-\unitvectorsym{s}^I)}
	\end{split}
\end{align}
\\
Note that we are using the convention that the weights are normalized to unity.
