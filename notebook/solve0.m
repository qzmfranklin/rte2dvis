...
{RULE1,RULE2,NU,NV}={19,5,2Nd+5,3};
...
ClearAll[V0]
V0 = Table[area[[n]] Exp[-I m phis], {n, Ns}, {m, -Nd, Nd}] // 
    Flatten // ToPackedArray;
V0 // PackedArrayQ;
ClearAll[X0]
X0 = LinearSolve[HomoMul[#, A, B, gTbl, \[Mu]t, \[Mu]s] &, V0, 
    Method -> {"Krylov", Tolerance -> 10^-20, Method -> "GMRES", 
	"Preconditioner" -> None, MaxIterations -> 50, 
"ResidualNormFunction" -> Norm}] // Quiet;
Print["Max Relative err=", 
  Abs[(HomoMul[X0, A, B, gTbl, \[Mu]t, \[Mu]s] - V0)/V0] // Quiet // 
    Flatten // Max];
ClearAll[data]
data = Re /@ (Partition[X0, 2 Nd + 1][[1, Nd + 1 ;; 2 Nd + 1]]) // 
   ToPackedArray;
(*ListPlot[data,PlotRange->{0,1/(2\[Pi])}]//Print;*) 
 ...
 *      "Preconditioner" -> Automatic, MaxIterations -> 50, 
...
CreateDirectory[dir]//Quiet;
SetDirectory[dir];
Export[ToString[Ns] <> "_" <> ToString[Nd], X0, "Complex128"];
